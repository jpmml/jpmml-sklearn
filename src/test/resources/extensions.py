...
def build_classifier(data, feature_pipeline, generations, population_size, name):
	...
	classifier.fit(Xt, y)
	pipeline = make_pmml_pipeline(Pipeline(steps = feature_pipeline.steps + classifier.fitted_pipeline_.steps), active_fields = X.columns.values, target_fields = [y.name])
	print(repr(pipeline))
